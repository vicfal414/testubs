...
<body>
    ...
    <?php
    echo ("made it here")
      $conn = mysql_connect('us-cdbr-east-02.cleardb.com','b33b6415873ff5','d1a1b9a1')
      if (!$conn) {
        die('Could not connect: ' . mysql_error());
      }
      $firstName = mysql_query('SELECT fname FROM accounts WHERE username=session.username', $conn)
      $lastName = mysql_query('SELECT lname FROM accounts WHERE username=session.username', $conn)
      echo ($firstName);
      echo ($lastName);
      mysql_close($conn);
    ?>
